...
function intisaliseUser($sessionid)
{
	...
	$sa = mysqli_escape_string($dbconn,$_SERVER['REMOTE_ADDR']); 
	if(isset($_SERVER['HTTP_REFERER']))
	{
		$sr = mysqli_escape_string($dbconn,$_SERVER['HTTP_REFERER']);
	}
	$sb = mysqli_escape_string($dbconn,$_SERVER['HTTP_USER_AGENT']);
...
function logConvo($response_Array)
{
	...
	$i = mysqli_escape_string($dbconn,$response_Array['usersaid']);
	$a = mysqli_escape_string($dbconn,$response_Array['biganswer']);
...
function replaceUndefined($originalinput,$bot)
{
	...
	if($result)
	{
		...
		{
			while($row=mysqli_fetch_array($result))//loop thru results
			{
...
function spellcheck($originalinput)
{
	...
	global $dbconn,$dbn,$debugmode;
	//for debug 
	$tmp = mysqli_escape_string($dbconn,$originalinput);
	//initialise
	$template="";
...
function formchat($response_Array)
{
  ...
  &nbsp;&nbsp;&nbsp;&nbsp;<form name=\"chat\" id=\"chat\" method=\"post\" action=\"\">
			<a name=\"chat\">&nbsp;</a>
			<div class=\"chat_submit_box\"><br><br>
    			<div class=\"uk-input-group\">
        			<div class=\"jarvis-chat-box\">
						<input type=\"text\" name=\"chat\" placeholder=\"Ask me anything.........\" style='width:195px;' id=\"chat\" size=\"35\" maxlength=\"50\">
						<input type=\"hidden\" name=\"action\" id=\"action\" value=\"checkresponse\">
					 </div>    
    						<span class=\"uk-input-group-addon\">
    							".$buildformvals."
								<input type=\"submit\" name=\"submit\" style=\"padding: 5px 16px;border-radius: 32px;border: 2px solid #189d0e;\" value=\"SAY\">
    						</span>
    			</div>
			</div>
