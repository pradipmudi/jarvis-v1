...
//INSTRUCTIONS//--------------------------------------------------------------------------------------
$step_one = "
<h2>Welcome to the J.A.R.V.I.S. installer.</h2>
<h3>Step 1/4</h3>
<p>Please make sure that you have installed the Program-O chatbot before you install J.A.R.V.I.S..</p>
...
<ol>
  ...
  <li>Create a MySQLI user with privileges to select, insert, update, delete and create tables.</li>
...
function adduser()
{
	...
	$u = mysqli_escape_string($dbconn,strip_tags(trim($_POST['u'])));
	$p = mysqli_escape_string($dbconn,strip_tags(trim($_POST['p'])));
	$cp = mysqli_escape_string($dbconn,strip_tags(trim($_POST['cp'])));
	...
	else
	{
			...
			$result = mysqli_query( $dbconn,$addadmin);
			...
			{
				$msg = "<p>There was an error when trying to add the user to the table.<br/>Check that your user has privileges and that the username does not already exist <a href=\"install_myprogramo.php?step=3\"> and then try again</a>.<br/>MySQL says: ".mysqli_error($dbconn)."</p>";
			}
...
`uname`
	...
	$result = mysqli_query( $dbconn,$installsql);
	...
	{
		$msg = "<p>There was an error when trying to create the table.<br/>Check that your user has privileges and that the table does not already exist <a href=\"install_myprogramo.php?step=2\">and then try again</a>.<br/>MySQL says: ".mysqli_error($dbconn)."</p>";
	}
...
<title>J.A.R.V.I.S. Installer</title>
</head>
<body>
